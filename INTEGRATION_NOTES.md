...
| Swap UI | We deep-link to **Cypher AMM V4** (`app.cyphereth.com/swap`). | `SwapInterface` is V4-aware (Algebra Integral). **Could replace the deep-link** with an on-site swap UI calling the Cypher Router V4 (`0x20C5893f…0b0Ab`). |
...
Today: button opens **app.cyphereth.com/swap** with input/output tokens pre-filled (Cypher AMM V4 = Algebra Integral v1.2).
After: on-site swap UI calling the Cypher Router V4 (`0x20C5893f69F635f55b0367C519F3f95e59c0b0Ab`) directly, so people don't bounce out. We don't take a fee — the cabal funds itself via the **Donate / Contribute** flow (separate page), which records public on-chain donations + notes from holders.

Real engineering — Algebra V4 routing has plugins (adaptive fee, sliding fee, etc.) that the SwapInterface has to encode. Defer unless the deep-link UX is hurting conversion.
